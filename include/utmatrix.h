...
{
	if (static_cast<unsigned long long>(s*s) > this->max_size) throw "Size is too big";
	for (int i = 0; i < s; ++i)
	{
...
{
	if (mt.getSize() != this->size)
		return false;
	for (int i = 0; i < mt.getSize(); ++i)
	{
		if (this->getElement(i) != mt.getElement(i))
			return false;
	}
	return true;
}
...
{
	if (this->size != mt.getSize() || *this != mt)
	{
