...
public:
	...
	{
		for (size_t i = v.getStartIndex(); i < v.size - v.startIndex; i++)
		{
			in >> v.pVector[i];
		}
		return in;
...
{
	if (_size < 0 || _size > max_size || _startIndex < 0 || _startIndex >= _size)
	{
		throw "_size or _startIndex out of range";
	}
	size = _size;
...
{
	if (_size > max_size || _startIndex >= _size)
	{
		throw "_size or _startIndex out of range";
	}
	size = _size;
