...
TEST(TMatrix, throws_when_try_get_element_with_too_large_index)
{
    TMatrix<int> m(3);

    ASSERT_ANY_THROW(m.getElement(4));
}

TEST(TMatrix, throws_when_try_get_element_with_index_larger_than_start_index)
{
    TMatrix<int> m(3);

    ASSERT_ANY_THROW(m[2].getElement(1));
}
...
TEST(TMatrix, compare_not_equal_matrices_return_false)
{
    TMatrix<int> m1(3);
    TMatrix<int> m2(3);
    m1[0][0] = 1; m2[0][0] = 2;
